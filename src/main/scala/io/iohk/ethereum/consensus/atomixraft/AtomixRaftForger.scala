...
class AtomixRaftForger(
  ...
  private[this] var _forgedTransactions: Long = 0L
  private[this] var _leaderships: Long = 0L
  private[this] var _forgedBlockLastTimeNanos: Long = 0L
  ...
  private def syncTheBlock(block: Block): Unit = {
    if(isLeader) {
      ...
      // Measure the block(s)
      val forgedBlocksDelta = 1
      this._forgedBlocks += forgedBlocksDelta

      metricsClient.gauge(Metrics.RaftLeaderForgedBlocksNumber, this._forgedBlocks)
      metricsClient.count(Metrics.RaftLeaderForgedBlocksCounter, forgedBlocksDelta)

      if(this._forgedBlockLastTimeNanos != 0L) {
        val nanoTime = System.nanoTime()
        val dt = nanoTime - this._forgedBlockLastTimeNanos
        this._forgedBlockLastTimeNanos = nanoTime

        MetricsClient.get().time(Metrics.RaftLeaderLastForgedBlockTime)
      }

      // measure the transactions
      val forgedTransactionsDelta = block.header.
